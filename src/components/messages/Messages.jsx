// File: my-backend/src/components/messages/Messages.jsx
...
const Messages = () => {
        ...
        const enrichedMessages = await Promise.all(
          messages.map(async (message) => {
            try {
              ...
              console.log("📌 Médias récupérés pour le message :", message.id, mediaData); // ✅ Affiche les médias
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const messageResponse = await fetch('http://localhost:5000/api/messages/new', {
        method: 'POST',
        headers: {
